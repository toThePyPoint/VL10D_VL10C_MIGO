...
from other_functions import append_status_to_excel, delete_file, vl10d_process_data, run_excel_file_and_adjust_col_width, copy_df_column_to_clipboard, close_excel_file
...
if __name__ == "__main__":
    ...
    try:
        ...
        vl10d_df = vl10d_process_data(file_name_raw_data=paths["vl10d_raw_data"], file_name_cleaned_data=paths["vl10d_clean_data"])
        ...
        zsbe_load_and_export_data(session=sess1, file_path=str(paths['temp_folder']), file_name=paths['zsbe_data_vl10d'].name)
        ...
        # TODO: process the data
        vl10c_df = vl10d_process_data(file_name_raw_data=paths["vl10c_raw_data"], file_name_cleaned_data=paths["vl10c_clean_data"])
        ...
        open_one_transaction(session=sess1, transaction_name="ZSBE")
        # zsbe - load data and export it to excel file
        zsbe_load_and_export_data(session=sess1, file_path=str(paths['temp_folder']), file_name=paths['zsbe_data_vl10c'].name)
        # close Excel file which should be automatically opened
        time.sleep(5)
